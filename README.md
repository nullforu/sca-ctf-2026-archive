...
| ----------------------- | ------------------------------------------------------------------------------------- | ---------------------------------------------- | ------------------------------------------------: | ------------------------ | ---------------------------------- |
| Email address           | Keep the local-part boundary characters and domain; add a collision suffix.           | JSON, SQL, logs, TAR members                   |                164 unique values; 407 appearances | `[email-value]`          | `[masked-email]`                   |
| User name / nickname    | Korean names retain outer characters; other names retain first/last characters.       | User, Discord, and participant records         |              2,296 structured display-name fields | `SampleUser`             | `S***r`                            |
| Team name               | Apply the display-name rule only to participant-team data.                            | `play_data.json` teams and user fields         |                   107 teams; 154 user team fields | `SampleTeam`             | `S***m`                            |
| Discord identifier      | Replace numeric IDs; apply display-name masking to Discord names.                     | SQL, JSON, logs, TAR members                   |                    98 database connection records | `<discord-id>`           | `[REDACTED_ID]`                    |
| IPv4 address            | Mask middle octets. VM/challenge external-address metadata is retained by exception.  | Logs, JSON, JSONL, TAR members                 |                                                 - | `<IPv4>`                 | `first.***.***.last`               |
| OAuth / bearer token    | Replace OAuth query parameters, bearer tokens, JWTs, and credential fields.           | URLs, headers, JSON, JSONL, logs               |                   287 OAuth parameter appearances | `code=value&state=value` | `code=[REDACTED]&state=[REDACTED]` |
| AWS credentials         | Replace AWS access key IDs and credential-bearing fields.                             | CloudTrail, CloudWatch, inventory, TAR members |                     86,496 access-key appearances | `AKIA...`                | `[REDACTED_AWS_ACCESS_KEY]`        |
| Database password hash  | Use one valid bcrypt demo hash so the dump remains importable.                        | PostgreSQL SQL dump                            |                                                 - | `<bcrypt-hash>`          | bcrypt hash for `example-password` |
